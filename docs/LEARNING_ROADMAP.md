...
4. わからない場合はAIに質問をしていい、AIはどう編集すればいいかを教えるが直接書き換えは行わない
5. `/review-day` する（当日の完了条件と整合性だけ見る。実装は返さない）
6. `/review-day` が Pass なら `/next-day`。振り返りの答えはこのチャットで書く（ロードマップには書かない）
...
`latest` / `@latest` は書かない。入れる時点でその時の最新に近い具体版を調べ、その番号をコマンド・Compose・`package.json` / `go.mod` に書く。ツールを「または同等」で選んだら、選んだ版を下の表に追記する。

### このリポジトリの版（2026-09 時点）

下は当時入れた版の記録。これから足すものはこの表をコピーせず、その日の最新に近い版をピンする。

- Node `26.0.0` / pnpm `12.4.1`
- Go `1.27.1`
- PostgreSQL `17.5`
- React `19.2.8` / React DOM `19.2.8`
- Vite `8.2.2`（作成: `pnpm create vite@8.2.2 web --template react-ts`）
- TypeScript `6.0.2`
- MUI `9.3.1` / Emotion `11.14.x`
- `lib/pq` `1.12.3`
- oapi-codegen `v2.8.0`
- Redocly CLI `2.49.0`
...
振り返り（`/next-day` のあと、このチャットで答える。このファイルには書かない）:

domain / application / infra のうち 1 つを選び、「してはいけないこと」を一文で書いてください。
...
- PostgreSQL 17.5 のサービスを 1 つ定義する（`postgres:17.5`。`latest` は使わない）
...
パスワードを環境変数にした理由と、ボリュームが再起動後に何を残すかを書いてください。
...
- `api/` に Go モジュールを初期化する（Go 1.25.6）
...
health の JSON を domain に書かなかった理由を書いてください。

翌日の入口: Day 4 で Vite 雛形を入れる
...
- 次で Vite + React + TypeScript の雛形を入れる（`latest` は使わない）
  - `pnpm create vite@8.2.2 web --template react-ts`
- Day 1 で `web/` が空でないと create は失敗する。一時ディレクトリに同じコマンドで作り、`package.json` / Vite 設定 / `src` の雛形だけ `web/` に移す
- Day 1 の `features` / `providers` / `lib` は消さない
- MUI 9.3.1 を入れ、画面に見出し 1 つを出す
- Compose に web サービスを追加する（開発サーバ。Node 26.0.0）
- API はまだ呼ばない
...
Day 1 の `features` などを消さずに、Vite のどのファイルを `web/` に移しましたか。
...
シークレットが git に入っていないことを、どのコマンドで確認しましたか。
...
health のパスを先に OpenAPI に書いた理由を書いてください。
...
仕様だけ見て、`POST /todos` が何を受けて何を返すか説明してください。
...
- oapi-codegen v2.8.0（または同等。選んだら「固定バージョン」表に版を追記）の設定を `api/` に置く
...
- orval / openapi-typescript 7.13.0 など、1 つ選んで固定する。選んだら「固定バージョン」表に版を追記する。`latest` は使わない
...
API と Web で同じになった型名を 1 つ挙げてください。
...
「仕様が単一ソース」とは、今日の構成ではどのファイルが本体ですか。
...
同じ migrate を二度流しても壊れない理由を書いてください。
...
SQL を handler に書かなかった理由を書いてください。
...
Insert が返した id を、なぜ List と FindByID の両方で確認しましたか。
...
usecase が `net/http` を import してはいけない理由を書いてください。
...
空タイトルを 400 にする判断は、handler と usecase のどちらが持ちますか。
...
curl で作った行と DB の行が一致することを、どう確認しましたか。
...
空タイトルを送れなくしたのは、画面・API のどちら（または両方）ですか。
...
削除した行が一覧と DB の両方から消えたことを、どう確認しましたか。
...
空状態と API エラー状態の違いを、画面の文言で説明してください。
...
- Vitest と Testing Library を入れる（入れる日の最新に近い版をピンし、表に追記。`latest` は使わない）
...
JWT の検証を usecase に置いてはいけない理由を書いてください。
...
今日取ったトークンの issuer と audience は何ですか。
...
HS256 から JWKS に替えたとき、触った層はどこですか。
...
ログアウトしたあと、一覧 API が 401 になる理由を書いてください。
...
他人の ToDo を 404 にする理由を書いてください（403 ではない理由）。
...
エラー JSON に出してはいけない情報を 1 つ挙げてください。
...
usecase テストが落ちたとき、メッセージから何が分かれば十分ですか。
...
- Playwright（または同等）を 1 つ選んで固定する。選んだら「固定バージョン」表に版を追記する。`latest` は使わない
...
E2E が落ちたとき、どの操作まで成功したか分かるようにした方法は何ですか。
...
format / lint / test のうち、意図的に壊すと先に落ちるのはどれですか。
...
- Node と Go は、入れる日の最新に近い版を workflow に書く（表に追記。`latest` は使わない）
...
workflow にシークレットを直書きしてはいけない理由を書いてください。
...
プール ID を README に書かない理由を書いてください。
...
- 期限やメモ、ラベル機能など、ToDo の最小列以外を 1 つだけ足す
